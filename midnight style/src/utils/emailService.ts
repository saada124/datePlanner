...
🌌 ${APP_CONFIG.girlfriendName.toUpperCase()} HAS LOCKED IN YOUR MIDNIGHT DATE! ✨❤️
...
💬 Starlight Message from ${APP_CONFIG.girlfriendName}:
"${selection.customNotes || "Ready for our magical date! ✨❤️"}"

Sent automatically from ${APP_CONFIG.girlfriendName}'s Cozy Midnight Starlight Web App.
`.trim();

  const payload = {
    _subject: `✨ ${APP_CONFIG.girlfriendName} Confirmed Our Date! (${selection.dayDate}) 🌌`,
    ...
    starlightNote: selection.customNotes || "None",
    message: midnightReport,
    _template: "table",
    _captcha: "false"
  };
