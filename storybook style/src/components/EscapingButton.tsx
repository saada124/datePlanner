...
export const EscapingButton: React.FC<EscapingButtonProps> = ({ onAttempt }) => {
  ...
  const [breezes, setBreezes] = useState<BreezeParticle[]>([]);
  ...
  const triggerEscape = useCallback((cursorX?: number, cursorY?: number) => {
    ...
    if (btn) {
      ...
      const newBreeze: BreezeParticle = {
        id: Date.now() + Math.random(),
        x: rect.left + rect.width / 2,
        y: rect.top + rect.height / 2
      };
      setBreezes(prev => [...prev.slice(-3), newBreeze]);
    }
  ...
  return (
    <div className="relative inline-block select-none">
      <AnimatePresence>
        {breezes.map(b => (
          <motion.div
            ...
            animate={{ opacity: 0, scale: 1.6, y: -25, rotate: 20 }}
            exit={{ opacity: 0 }}
            transition={{ duration: 0.5, ease: "easeOut" }}
            className="fixed pointer-events-none z-50 font-handwriting text-sm text-storybook-roseDark bg-storybook-blush px-3 py-1 rounded-full border border-storybook-rose shadow-sm"
      ...
      <motion.button
        ...
        className="story-btn-secondary px-6 py-3.5 text-xs sm:text-sm text-storybook-inkLight border-storybook-border hover:border-storybook-rose cursor-not-allowed whitespace-nowrap relative z-20"
      >
